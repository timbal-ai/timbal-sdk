...
describe("QueryService", () => {
  ...
  describe("query", () => {
    ...
    test("should throw error when orgId is missing", async () => {
      try {
        ...
        expect((error as Error).message).toBe('orgId is required. Provide it in the method call or set a default.');
      }
    });
    ...
    test("should throw error when kbId is missing", async () => {
      try {
        ...
        expect(error).toBeInstanceOf(Error);
        expect((error as Error).message).toBe('kbId is required. Provide it in the method call or set a default.');
      }
    });
    ...
    test("should throw error when sql is missing", async () => {
      try {
        ...
        expect(error).toBeInstanceOf(Error);
        expect((error as Error).message).toBe('sql is required. Provide it in the method call or set a default.');
      }
    });
  });
  ...
  describe("queryByParams", () => {
    test("should execute query with positional parameters", async () => {
      const result = await queryService.queryByParams(
        "123",
        "456", 
        'SELECT * FROM "Users"'
      );
  ...
  describe("defaults", () => {
    test("should use default values", async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        "orgs/default-org/kbs/default-kb/query",
        { sql: 'SELECT 1' }
      );
    });
    ...
    test("should override defaults with provided values", async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(
        "orgs/override-org/kbs/default-kb/query",
        { sql: 'SELECT 1' }
      );
    });
...
describe("QueryService Integration Tests", () => {
  ...
  test.skipIf(!shouldRunIntegrationTests())("should handle query with parameters from environment", async () => {
  ...
  test.skipIf(!shouldRunIntegrationTests())("should handle multiple concurrent queries", async () => {
    const queries = Array(3).fill(null).map((_, i) => 
      timbal.query({
