...
//   TIMBAL_INTEGRATION_TOKEN        bearer token (or TIMBAL_API_KEY / ~/.timbal)
...
/** Bun defaults to 5s — multipart upload + signed-URL fetch need more headroom. */
...
describe.skipIf(SKIP)('integration: temp files (POST /files)', () => {
  test(
    'uploadTempFile returns TempFile with url + expires_at, no id',
    async () => {
  ...
  test(
    'uploadTempFileFromBuffer round-trips bytes through the signed URL',
    async () => {
      const timbal = makeTimbal();
  ...
  test(
    'does not require orgId in client config',
    async () => {
      const t = new Timbal({
        baseUrl: BASE_URL ?? 'https://api.timbal.ai',
        ...(TOKEN && { token: TOKEN }),
...
describe.skipIf(SKIP)('integration: org-bucket files (deprecated)', () => {
  test(
    'uploadFile coerces id to string',
    async () => {
      const timbal = makeTimbal();
      if (!hasCreds(timbal)) return;
      if (!ORG_ID) {
  ...
  test(
    'uploadFileFromBuffer coerces id to string',
    async () => {
      const timbal = makeTimbal();
