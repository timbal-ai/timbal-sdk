...
export const shouldRunIntegrationTests = () => {
  ...
  return !!(process.env.TIMBAL_API_KEY && process.env.TIMBAL_ORG_ID && process.env.TIMBAL_KB_ID);
};
...
export const logIntegrationTestConfig = () => {
  if (shouldRunIntegrationTests()) {
    ...
    console.log(
      '⚠️  Skipping integration tests - set TIMBAL_API_KEY, TIMBAL_ORG_ID, and TIMBAL_KB_ID environment variables to run'
    );
  }
};
