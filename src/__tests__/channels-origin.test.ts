...
import { detectNgrokOrigin, resolvePublicOrigin } from '../channels/origin';
...
describe('resolvePublicOrigin', () => {
  test('explicit origin wins over everything', async () => {
    const origin = await resolvePublicOrigin({
      ...
      env: { PUBLIC_ORIGIN: 'https://env.example.com' },
      fetchImpl: fetchThrowing,
    });
    expect(origin).toBe('https://explicit.example.com');
  });

  test('PUBLIC_ORIGIN env beats the tunnel probe', async () => {
    const origin = await resolvePublicOrigin({
      env: { PUBLIC_ORIGIN: 'https://env.example.com' },
      fetchImpl: fetchThrowing,
    });
  ...
  test('platform-linked (TIMBAL_PROJECT_ID) never probes tunnels', async () => {
    let probed = false;
