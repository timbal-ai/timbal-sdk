...
describe('FileService', () => {
  ...
  test('should handle positional parameters', async () => {
    ...
    const result = await fileService.uploadFileByParams(
      'test-org',
      tempFilePath
    );
  ...
  test('should throw error when buffer data is missing', async () => {
    ...
    await expect(fileService.uploadFileFromBuffer({
      orgId: 'test-org',
      data: null as any,
      filename: 'test.txt'
...
describe("FileService Integration Tests", () => {
  ...
  afterEach(async () => {
    ...
    console.log(`ℹ️  ${uploadedFiles.length} files were uploaded during tests (no cleanup available)`);
    uploadedFiles.length = 0; // Clear the array
  });

  test.skipIf(!shouldRunIntegrationTests())("should upload a text file and verify response", async () => {
  ...
  test.skipIf(!shouldRunIntegrationTests())("should handle file upload errors gracefully", async () => {
    try {
